...
async def buy_beginner(update: Update, context: CallbackContext):
    ...
    # Выполняем отправку инвойса в фоновом режиме
    background_task(send_invoice, context.bot, chat_id, title, description, payload, provider_token, currency, prices)
...
def send_invoice(bot, chat_id, title, description, payload, provider_token, currency, prices):
